...
signed main()
{
    ...
    for (int i = 0; i < N; i++)
    {
        int x, y, h;
        cin >> x >> y >> h;
        data.push_back(make_pair(h, make_pair(x, y)));
    }

    sort(data.begin(), data.end(), greater<pair<int, pair<int, int>>>());
    ...
    for (int ax = 0; ax <= 100; ax++)
    {
        ...
        {
            ah = data[0].first + abs(data[0].second.first - ax) + abs(data[0].second.second - ay);
            bool is_ans = true;
            for (int k = 1; k < data.size(); k++)
            {
                if (max(ah - abs(data[k].second.first - ax) - abs(data[k].second.second - ay), 0) != data[k].first)
                {
                    is_ans = false;
                    break;
                }
