...
inline void setCostCl(float cl, double start, double end)
{
    setCostCl(0.);
    int m, d;
    for (int i = Nimportant * start; i < Nimportant * end; ++i)
    {
...
*/
    ...
    */
    ...
    for (int i = 0; i < population; ++i)
    {
        ...
        //setCostCl(uniformDist(mt), (double)i / (double)population, (double)(i + 1) / (double)population);
        forwardAdd();
        randomForwardReduce();
        randomForwardReduce();
        rpToTimes.push_back(pair<ll, vector<vector<int>>>(randomForwardReduce(), mdToAc));
    }
    ll tmp;
    while (checkTime())
    {
        for (int i = 0; i < population; ++i)
        {
            ...
            forwardAdd();
            randomForwardReduce();
            tmp = randomForwardReduce();
            if (tmp > rpToTimes[i].first)
            {
                rpToTimes[i].first = tmp;
