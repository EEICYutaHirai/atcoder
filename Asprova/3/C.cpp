...
inline void forwardAdd()
{
    shuffle(importantMd.begin(), importantMd.end(), mt);
    ll tmpEvaluation = prEvaluation();
    int d, m, prevAc;
    for (int i = 0; i < Nimportant; ++i)
    {
        m = importantMd[i].first;
        ...
        mdToAc[m][d] += max(MAX_Cost[m] * uniformDist(mt), 5.);

        forwardAssign();
        ll tmptmpEvaluation = prEvaluation();
        //cout << tmptmpEvaluation << endl;
        if (tmpEvaluation < tmptmpEvaluation)
        {
            tmpEvaluation = tmptmpEvaluation;
...
inline void forwardAdd(double cl)
{
    shuffle(importantMd.begin(), importantMd.end(), mt);
    ll tmpEvaluation = prEvaluation();
    int d, m, prevAc;
...
inline void forwardAddAll()
{
    ...
    for (int i = 0; i < Nimportant; ++i)
    {
        ...
        mdToAc[m][d] += max(MAX_Cost[m] * uniformDist(mt), 3.);
    }
    return;
}
    ...
    */
    ...
    while (checkTime())
    {
        ...
        for (int i = 0; i < nSurvive; ++i)
        {
            ...
            rpToTimes[i].first = backwardReduce();
        ...
        {
            mdToAc = rpToTimes[i].second;
            forwardAdd(uniformDist(mt));
            randomForwardReduce();
            rpToTimes[i].first = randomForwardReduce();
        ...
        {
            shuffle(vShuffleElete.begin(), vShuffleElete.end(), mt);
            int x = vShuffle[0];
            int y = vShuffle[1];
            for (int m = 0; m < M; ++m)
            {
