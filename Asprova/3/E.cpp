...
std::uniform_real_distribution<> uniformDist(0.0, 1.0), pm(-0.5, .5);
...
inline double conEvaluation()
{
    ...
    for (int r = 0; r < R; ++r)
    {
        ...
        if (rpToTime[r][p].second <= orders[r].d)
            ...
            //grossP -= (double)orders[r].pr / (double)(rpToTime[r][p].second - orders[r].d) + 10. * (double)(rpToTime[r][p].second - orders[r].d) * orders[r].pr / SECONDS_A_DAY;
...
double inline conBackwardReduce()
{
    forwardAssign();
...
int main()
{
    ...
    vector<vector<int>> bestMd = mdToAc;
    forwardAssign();
    double beste = conEvaluation();
    double dtmp;
    for (int i = 0; i < 10; ++i)
    {
    ...
    uniform_int_distribution<> ran(0, Nimportant - 1);
    int nc = Nimportant * 0.1;
    mdToAc = bestMd;
    while (currentTime < timeLimit)
    {
        for (int i = 0; i < 100; ++i)
        {
            index = randxor() % Nimportant;

            m = importantMd[index].first;
            d = importantMd[index].second;
            if (randxor() % Nimportant * 2 > Nimportant)
            {
                mdToAc[m][d]++;
            }
            else
