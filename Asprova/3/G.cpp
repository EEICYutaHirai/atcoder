...
inline double conEvaluation()
{
    ...
    for (int r = 0; r < R; ++r)
    {
        ...
        if (rpToTime[r][p].second <= orders[r].d)
            ...
            grossP -= (double)(rpToTime[r][p].second - orders[r].d) * (double)MAX_Pr / SECONDS_A_DAY / 3.;
    }
    double cost = 0;
    for (int m = 0; m < M; ++m)
    {
    ...
    for (int x = 0; x < N_IND; x++)
    {
        for (int m = 0; m < M; ++m)
        {
            for (int d = 1; d <= maxDead; ++d)
            {
                ...
                mdToAcs[x].second[m][d] = abs(mRandom[m](mt)) / (double)d;
            }
        }
    }
}
            ...
            for (int m = 0; m < M; ++m)
            {
                for (int d = 1; d <= maxDead; ++d)
                {
                    if (rdist(engine) < P_META)
                        nMdToAcs[n_keep + n_sex + x].second[m][d] = abs(mRandom[m](mt)) / d;
                    else
...
void runLocalSearch()
{
    ...
    while (duration_cast<std::chrono::milliseconds>(system_clock::now() - pStart).count() < END_TIME)
    {
        ...
        {
            for (int k = 0; k < N_LOCAL_DEPTH; k++)
            {
                //コスパ、m,d
                mdToAc = mdToAcs[n].second;
                solve();
                //clean():
                int originalEv = evaluation();
                ...
                for (int m = 0; m < M; ++m)
                {
                    ...
                    {
                        mdToAc = mdToAcs[n].second;
                        mdToAc[m][d] += maxCost[m];
                        solve();
                        data.push_back(pair<int, Pii>(evaluation(), Pii(m, d)));
                    }
                }
                sort(data.begin(), data.end(), greater<pair<int, Pii>>());
            }
        }
    }
}
